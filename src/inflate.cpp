...
Inflate::Inflate()
    : dis_hfm_tree_(new DisHuffman),
      ll_hfm_tree_(new LlHuffman),
      in_data_(0),
      in_bit_cnt_(0),
      out_buffer_(new uint8[I_O_BUFFSIZE]),
      out_buffer_cnt_(0),
      fp_out_(NULL),
      fp_in_(NULL) {}
...
void Inflate::Uncompress(const char* compressed_file_path_and_name,
  ...
  do {
    ...
    original_file_name[i++] = ch;
      ...
      ch !=
      '\0');  // when read the '\0', the flag of end of the original filename
    ...
    while (true) {
      ...
      if (encode_ll >= 0 && encode_ll <= 255) {
        ...
        if (out_buffer_cnt_ == I_O_BUFFSIZE) {
          ...
          memcpy(out_buffer_, out_buffer_ + I_O_BUFFSIZE / 2,
                 I_O_BUFFSIZE / 2 * sizeof(uint8));
        ...
        for (int32 i = 0; i < ll_hfm_tree_->ic_extra_bits_[encode_ll - 257];
             i++) {
          if (InBit()) extra |= (1 << i);
        }
        len += extra;
        ...
        if (dis < len) {
          uint32 rest_len = len;
          while (rest_len >= dis) {
            memcpy(out_buffer_ + out_buffer_cnt_,
                   out_buffer_ + out_buffer_cnt_ - dis, dis * sizeof(uint8));
            out_buffer_cnt_ += dis;
            rest_len -= dis;
          }
          memcpy(out_buffer_ + out_buffer_cnt_,
                 out_buffer_ + out_buffer_cnt_pre - dis,
                 rest_len * sizeof(uint8));
          out_buffer_cnt_ += rest_len;
        } else {
          memcpy(out_buffer_ + out_buffer_cnt_,
                 out_buffer_ + out_buffer_cnt_ - dis, len * sizeof(uint8));
          out_buffer_cnt_ += len;
        }
