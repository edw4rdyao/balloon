...
int DeflateWork(const char *filepath) {
  // parser the file_path into : drive letter, directory path, filename,
  // filename extension
  ...
  // splice the directory and the filename(whose extention name is .bal) to open
  // the copmressed file
  strcat(directory, filename);
  const char *balloon_extention = ".bal";
  strcat(directory, balloon_extention);
  // splice the filename and its original extention name, which is to be saved
  // in compressed file
...
int InflateWork(const char *filepath) {
  // parser the filepath into : drive letter, directory path, filename, filename
  // extension
  char drive[_MAX_DRIVE];
  char directory[_MAX_DIR];
  char filename[_MAX_FNAME];
  ...
  if (strcmp(extention, ".bal")) {
    std::cout << "Sorry, ballon only supports the '.bal' file format to be "
                 "uncompressed!\n";
    return 0;
  }
...
void DisplayHelpInformation() {
  ...
  std::cout << "Options:\n";
  std::cout << std::left << "\t" << std::setw(30) << "--help or -h"
            << "display the information of usage.\n";
  std::cout << std::left << "\t" << std::setw(30) << "--version or -v"
            << "display the version information of the balloon.exe and detail "
               "of the program.\n";
  std::cout << std::left << "\t" << std::setw(30) << "--deflate or -d <args>"
            << "compress the file. <args>: the file path that you want to "
               "compress.\n";
  std::cout << std::left << "\t" << std::setw(30) << "--inflate or -i <args>"
            << "uncompress the file. <args>: the file path that you want to "
               "uncompress.\n";
  return;
}
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    std::cout << argv[0] << ":"
              << " please use option [--help] to get more information.";
  }
  // using getopt_long function to get every option user input until return -1
  while ((option = getopt_long(argc, argv, single_options_string, long_options,
                               &option_index)) != -1) {
    switch (option) {
      case 'd':
